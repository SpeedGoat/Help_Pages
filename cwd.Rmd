...
The estimates table displays model outputs (estimated latent prevalence) for all classes (Collection Method/Age/Sex) in a single table. Mean, median, standard deviation, and confidence intervals (90% and 95%) are reported for each class. Rows are color coded based on the number of samples entered for each class.
...
The plots page shows a probability density plot for estimated latent prevalence of CWD by class, along with trace and autocorrelation plots for diagnosing model performance. Change the data displayed by using the dropdown below the plot to select a different class. You can download all density plots as image files or in a single pdf using the *Download All* button.
