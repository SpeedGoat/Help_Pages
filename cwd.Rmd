...
The plots page shows a posterior distribution plot for estimated latent prevalence of CWD by class, along with trace and autocorrelation plots for diagnosing model performance. Change the data displayed by using the dropdown below the plot to select a different class. Use the buttons and sliders near the various plots to zoom in to a finer scale or save a plot as an image file. You can also download all density plots in a single click using the *Download All* button.
